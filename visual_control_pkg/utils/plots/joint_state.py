...
FIGSIZE = (10, 6)
JS_M_KEYS = ["JS (Position)", "JS (Velocity)"]
...
def plot_joint_position(
    ...
    fig.supylabel(r"Joint Position (rad)")
    axes[0].legend()
    for i, ax in enumerate(axes):
    ...
    plt.savefig(save_path / "js_position.png", dpi=DPI)
    plt.show()
...
def plot_joint_velocity(
    ...
    save_path = Path(__file__).parent / "figures"
    save_path.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path / "js_velocity.png", dpi=DPI)
    plt.show()
...
def main(paths: list[str], ctrls: list[str]):
    all_readings = []
    for path in paths:
        readings = read_csv_log(path, filters=["JS"])
        readings = normalize_steps(readings)
        all_readings.append(readings)
    plot_joint_position(ctrls, all_readings)
    plot_joint_velocity(ctrls, all_readings)

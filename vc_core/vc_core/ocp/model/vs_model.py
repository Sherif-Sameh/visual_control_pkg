...
def export_vs_ode_model(fp: NDArray, alpha: float = 0.001) -> AcadosModel:
    ...
    The model's stage cost in made up of three groups of terms: state `x`, input `u_dot` and visual
    state `z`. The visual state is made up of the 2D feature location `s` and its rate of change
    `s_dot`. The terminal cost consists of only a single term of the state `x`.
    ...
    cost_y = ca.vertcat(p_err, q_err, u, u_dot, z)

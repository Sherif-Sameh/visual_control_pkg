...
def plot_joint_trajectory(
    ...
    fig.supylabel(r"Joint Trajectory (rad/s)")
    axes[0].legend()
    for i, ax in enumerate(axes):
    ...
    plt.savefig(save_path / "jt_velocity.png", dpi=DPI)
    plt.show()
...
def main(paths: list[str], ctrls: list[str]):
    all_readings = []
    for path in paths:
        readings = read_csv_log(path, filters=["JT"])
        readings = normalize_steps(readings)
        all_readings.append(readings)
    plot_joint_trajectory(ctrls, all_readings)

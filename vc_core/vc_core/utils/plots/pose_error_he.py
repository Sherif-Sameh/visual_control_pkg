...
COLORS = {"ideal": "#F1750F", "noisy_1": "#D426C2", "noisy_2": "#0E4DAB", "hline": "#66706e"}
XLIM = [0, 56]
...
def plot_position_errors(
    ...
    LABELS = {
        "ideal": r"Ideal",
        "noisy_1": r"Biased ($\varepsilon_{\mathbf{p}} = 0.5$cm)",
        "noisy_2": r"Biased ($\varepsilon_{\mathbf{p}} = 1$cm)",
    }
    ...
    plt.savefig(save_path / "pe_position_he.png", dpi=DPI)
    plt.show()
...
def plot_rotation_errors(
    ...
    LABELS = {
        "ideal": r"Ideal",
        "noisy_1": r"Biased ($\varepsilon_{\boldsymbol{\tau}} = 1^{\circ}$)",
        "noisy_2": r"Biased ($\varepsilon_{\boldsymbol{\tau}} = 2^{\circ}$)",
    }
    ...
    save_path = Path(__file__).parent / "figures"
    save_path.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path / "pe_rotation_he.png", dpi=DPI)
    plt.show()


def main(p_id: str, p_n1: str, p_n2: str):
    paths = [p_id, p_n1, p_n2]
    ctrls = ["ideal", "noisy_1", "noisy_2"]
    all_readings = []
    for path in paths:

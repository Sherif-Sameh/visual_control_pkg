...
def declare_arguments() -> list[DeclareLaunchArgument]:
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "wandb_group",
            default_value="TCPCalib|P3D|ideal",
            description="Group name for run to use for WandB logger. Default is TCPCalib|P3D|ideal.",
        )
    )
...
def generate_launch_description() -> LaunchDescription:
    ...
    csv_dir = PathJoinSubstitution(
        [FindPackageShare("logging_pkg"), "../../../../logs/csv/tcpcalib_p3d"]
    )
    wandb_dir = PathJoinSubstitution(
        [FindPackageShare("logging_pkg"), "../../../../logs/wandb/tcpcalib_p3d"]
    )
    ...
    config_path = os.path.join(pkg_share, "config", "tcpcalib_p3d_logger.toml")
    config = toml.load(config_path)

    # Initialize nodes to start
    tcpcalib_p3d_logger_node = Node(
        package="logging_pkg",
    ...
    nodes_to_start = [tcpcalib_p3d_logger_node]

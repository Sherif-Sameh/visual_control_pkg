...
def declare_arguments() -> list[DeclareLaunchArgument]:
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "wandb_group",
            default_value="HEEVal|ideal",
            description="Group name for run to use for WandB logger. Default is HEEVal|ideal.",
        )
    )
...
def generate_launch_description() -> LaunchDescription:
    ...
    csv_dir = PathJoinSubstitution([FindPackageShare("logging_pkg"), "../../../../logs/csv/heeval"])
    wandb_dir = PathJoinSubstitution(
        [FindPackageShare("logging_pkg"), "../../../../logs/wandb/heeval"]
    )
    ...
    config_path = os.path.join(pkg_share, "config", "heeval_logger.toml")
    config = toml.load(config_path)

    # Initialize nodes to start
    heeval_logger_node = Node(
        package="logging_pkg",
    ...
    nodes_to_start = [heeval_logger_node]
